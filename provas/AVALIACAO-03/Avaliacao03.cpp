...
model body, front_w, back_w, fork;
...
void renderModel(const model &model) {
  ...
  for (int i = 0; i < model.faces.size(); ++i) {
    ...
    const vertex &t1 = (model.textures.empty())
                           ? vertex()
                           : model.textures[model.faces[i].t1 - 1];
    const vertex &t2 = (model.textures.empty())
                           ? vertex()
                           : model.textures[model.faces[i].t2 - 1];
    const vertex &t3 = (model.textures.empty())
                           ? vertex()
                           : model.textures[model.faces[i].t3 - 1];
...
void init() {
  ...
  loadObj("frame.obj", body);
  loadObj("fork.obj", fork);
  loadObj("front_w.obj", front_w);
  loadObj("back_w.obj", back_w);
}
...
void drive() {
  ...
  if (steer > 45) turn = false;

  if (steer < -45) turn = true;
